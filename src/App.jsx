...
// ══════════════════════════════════════════════
//  EXERCISE GIF LIBRARY (external dataset)
// ══════════════════════════════════════════════
const LIB_URL = "/data/exercises-lib.json";
const LIB_GIF_BASE = "https://cdn.jsdelivr.net/gh/hasaneyldrm/exercises-dataset@main/";
const LIB_ATTRIBUTION = "GIFs e imágenes: © Gym visual — gymvisual.com";
...
const BODY_PART_ES = {
  ...
  "lower legs":"Pantorrillas", neck:"Cuello", shoulders:"Hombros", "upper arms":"Brazos",
  "upper legs":"Piernas", waist:"Abdomen",
};

const ES_EN_TERMS = {
  sentadilla:"squat", sentadillas:"squat", dominadas:"pull-up", dominada:"pull-up",
  remo:"row", jalon:"pulldown", jalón:"pulldown", extension:"extension", extensión:"extension",
  elevacion:"raise", elevación:"raise", zancada:"lunge", zancadas:"lunge", empuje:"push",
  tiron:"pull", tirón:"pull", pecho:"chest", espalda:"back", hombro:"shoulder", hombros:"shoulders",
  biceps:"biceps", "bíceps":"biceps", triceps:"triceps", "tríceps":"triceps", pierna:"leg",
  piernas:"legs", gluteo:"glute", "glúteo":"glute", gluteos:"glutes", "glúteos":"glutes",
  pantorrilla:"calf", pantorrillas:"calves", abdomen:"abs", abdominales:"abs", muerto:"deadlift",
  banca:"bench", inclinado:"incline", mancuerna:"dumbbell", mancuernas:"dumbbell", barra:"barbell",
  polea:"cable", sentado:"seated", frontal:"front", lateral:"lateral", cuadriceps:"quad",
  "cuádriceps":"quad", femoral:"hamstring", gemelos:"calf", plancha:"plank", fondos:"dip",
  abduccion:"abduction", "abducción":"abduction", press:"press", curl:"curl",
};
function translateQuery(q) {
  return q.toLowerCase().trim().split(/\s+/).map(w => ES_EN_TERMS[w] || w).join(" ");
}

// Module-level cache so the ~1MB dataset is fetched only once per app session
let LIB_CACHE = null, LIB_PROMISE = null;
function useLibrary() {
  const [lib, setLib] = useState(LIB_CACHE);
  useEffect(() => {
    if (LIB_CACHE) { setLib(LIB_CACHE); return; }
    if (!LIB_PROMISE) LIB_PROMISE = fetch(LIB_URL).then(r => r.json()).then(d => { LIB_CACHE = d; return d; }).catch(() => { LIB_CACHE = []; return []; });
    let alive = true;
    LIB_PROMISE.then(d => { if (alive) setLib(d); });
    return () => { alive = false; };
  }, []);
  const byId = useMemo(() => {
    const m = {};
    (lib || []).forEach(e => { m[e.id] = e; });
    return m;
  }, [lib]);
  return { lib, byId };
}
function libEntryFor(byId, ourLibId) {
  const dsId = EX_LIB_MAP[ourLibId];
  return dsId ? byId[dsId] : null;
}
function gifSrc(path) { return path ? `${LIB_GIF_BASE}${path}` : null; }
...
// GIF thumbnail — small preview used in lists/cards. `size` is px number, or "100%" for a responsive square.
function GifThumb({ entry, size = 44, onClick }) {
  const dim = typeof size === "string" ? { width:size, aspectRatio:"1 / 1", height:"auto" } : { width:size, height:size };
  const fontSize = typeof size === "string" ? 24 : size*0.45;
  if (!entry) return (
    <div style={{ ...dim, borderRadius:8, background:C.s3, display:"flex", alignItems:"center", justifyContent:"center", fontSize, flexShrink:0, color:C.dimmer }}>🏋️</div>
  );
  return (
    <img src={gifSrc(entry.gif_url)} loading="lazy" alt="" onClick={onClick}
      style={{ ...dim, borderRadius:8, objectFit:"cover", background:C.s3, flexShrink:0, cursor:onClick?"pointer":"default", border:`1px solid ${C.b1}` }}
      onError={e => { e.target.style.visibility="hidden"; }} />
  );
}

// Full-size GIF + instructions modal
function GifDetailModal({ open, onClose, entry }) {
  if (!open || !entry) return null;
  return (
    <Modal open={open} onClose={onClose} title={entry.name}>
      <img src={gifSrc(entry.gif_url)} alt={entry.name} style={{ width:"100%", maxWidth:280, display:"block", margin:"0 auto 14px", borderRadius:10, background:C.s2 }} />
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:14 }}>
        <Tag active>{BODY_PART_ES[entry.body_part] || entry.body_part}</Tag>
        <Tag active color={C.blue}>{entry.equipment}</Tag>
        {entry.target && <Tag active color={C.yellow}>{entry.target}</Tag>}
      </div>
      {entry.secondary_muscles?.length > 0 && (
        <div style={{ fontSize:11, color:C.dim, marginBottom:14 }}>Músculos secundarios: {entry.secondary_muscles.join(", ")}</div>
      )}
      {entry.instructions_es?.length > 0 && (
        <div style={{ marginBottom:10 }}>
          <Lbl>Instrucciones</Lbl>
          <ol style={{ paddingLeft:18, display:"flex", flexDirection:"column", gap:8, margin:0 }}>
            {entry.instructions_es.map((s,i) => <li key={i} style={{ fontSize:13, color:C.dim, lineHeight:1.5 }}>{s}</li>)}
          </ol>
        </div>
      )}
      <div style={{ fontSize:9, color:C.dimmer, marginTop:16 }}>{LIB_ATTRIBUTION}</div>
    </Modal>
  );
}
...
// ══════════════════════════════════════════════
function ExerciseAgendaModal({ open, onClose, exercise, logs, routines, byId }) {
  ...
  const gifEntry = byId ? libEntryFor(byId, exercise.libId) : null;

  return (
    <Modal open={open} onClose={onClose} title={libEx?.name || ""}>
      <GifDetailModal open={showGif} onClose={() => setShowGif(false)} entry={gifEntry} />
      {gifEntry && (
        <div style={{ display:"flex", justifyContent:"center", marginBottom:16 }} onClick={() => setShowGif(true)}>
          <GifThumb entry={gifEntry} size={100} onClick={() => setShowGif(true)} />
        </div>
      )}
      {/* Muscle + PR */}
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
...
// ══════════════════════════════════════════════
function ExConfig({ ex, logs, rid, routines, onRemove, onSetCount, onSetWeight, byId }) {
  const libEx = exById(ex.libId);
  const gifEntry = byId ? libEntryFor(byId, ex.libId) : null;
  ...
  return (
    <>
      <ExerciseAgendaModal open={showAgenda} onClose={() => setShowAgenda(false)} exercise={ex} logs={logs} routines={routines} byId={byId} />
      <Card style={{ background:C.s2, borderColor:C.b2, marginBottom:10 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:open?12:0 }}>
          <div style={{ flex:1, display:"flex", gap:10, alignItems:"flex-start", cursor:"pointer" }} onClick={() => setOpen(p=>!p)}>
            <GifThumb entry={gifEntry} size={40} />
            <div style={{ flex:1 }}>
...
export default function App() {
  ...
  async function loadAll(uid) {
    const [r,l,p,m] = await Promise.all([
      ...
      dbGet("metrics",`user_id=eq.${uid}&order=date.desc`),
    ]);
    setRoutines(r); setLogs(l); setPrograms(p); setMetrics(m);
  }
  ...
  async function handleLogout() {
    TOKEN=null; clearS();
    setUser(null); setRoutines([]); setLogs([]); setPrograms([]); setMetrics([]);
    setActiveSess(null); setView("dashboard"); setStatus("anon");
  }

  const db = {
    routines, logs, programs, metrics,
    addRoutine: async d => {
    ...
    addMetric: async d => {
      const row = { ...d, id:genId(), user_id:user.id, date:d.date||new Date().toISOString() };
      const { ok } = await dbIns("metrics", row);
      if (ok) { setMetrics(p=>[row,...p].sort((a,b)=>new Date(b.date)-new Date(a.date))); showToast("Registro guardado ✓"); return row; }
      else { showToast("Error al guardar","err"); return null; }
    },
    deleteMetric: async id => {
      await dbDel("metrics", id);
...
function RoutinesView({ db, showToast }) {
  ...
  if (building) return (
    <div>
      ...
      {building.exercises.length>0 && (
        <div style={{ marginBottom:20 }}>
          ...
          {building.exercises.map(ex => (
            <ExConfig key={ex.libId} ex={ex} logs={db.logs} rid={building.id||"new"} routines={db.routines} byId={byId}
      ...
      {filtLib.map(le => {
        ...
        const gifEntry = libEntryFor(byId, le.id);
        return (
          <div key={le.id} onClick={()=>toggleEx(le.id)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, padding:"11px 14px", marginBottom:5, cursor:"pointer", borderRadius:10, background:sel?"rgba(255,255,255,0.05)":C.s1, border:`1px solid ${sel?C.b3:C.b1}` }}>
            <GifThumb entry={gifEntry} size={36} />
            <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:500 }}>{le.name}</div><div style={{ fontSize:10, color:C.dim, marginTop:2 }}>{le.muscle}{le.compound?" · ★":""}</div></div>
  ...
  return (
    <div>
      ...
      {agendaEx && (
        <ExerciseAgendaModal open={!!agendaEx} onClose={()=>setAgendaEx(null)} exercise={agendaEx} logs={db.logs} routines={db.routines} byId={byId}/>
      )}
        ...
        return (
          <Card key={r.id} style={{ marginBottom:10 }}>
            ...
            {(r.exercises||[]).map(ex => {
              ...
              const gifEntry = libEntryFor(byId, ex.libId);
              ...
              return (
                <div key={ex.libId} style={{ padding:"9px 0", borderTop:`1px solid ${C.b1}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                    <GifThumb entry={gifEntry} size={32} onClick={()=>setAgendaEx({libId:ex.libId})} />
                    <div style={{ flex:1 }}>
...
function SessionView({ db, session, setSession, setView, showToast }) {
  ...
  return (
    <div>
      <GifDetailModal open={!!gifEx} onClose={()=>setGifEx(null)} entry={gifEx} />
      <div style={{ marginBottom:20 }}>
          ...
          <Card key={ex.libId} style={{ marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8, gap:10 }}>
              <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                <GifThumb entry={gifEntry} size={48} onClick={()=>gifEntry&&setGifEx(gifEntry)} />
...
// ══════════════════════════════════════════════
//  LIBRARY — explorador de 1.324 ejercicios con GIF
// ══════════════════════════════════════════════
const PAGE_SIZE = 24;
function LibraryView() {
  const { lib, byId } = useLibrary();
  const [search, setSearch] = useState("");
  const [bodyFilter, setBodyFilter] = useState("Todos");
  const [visible, setVisible] = useState(PAGE_SIZE);
  const [detail, setDetail] = useState(null);

  const bodyParts = useMemo(() => {
    if (!lib) return [];
    return ["Todos", ...new Set(lib.map(e => e.body_part))];
  }, [lib]);

  const filtered = useMemo(() => {
    if (!lib) return [];
    let arr = lib;
    if (bodyFilter !== "Todos") arr = arr.filter(e => e.body_part === bodyFilter);
    if (search.trim()) {
      const terms = translateQuery(search).split(/\s+/).filter(Boolean);
      arr = arr.filter(e => {
        const hay = `${e.name} ${e.target} ${e.muscle_group} ${e.equipment} ${(e.secondary_muscles||[]).join(" ")}`.toLowerCase();
        return terms.every(t => hay.includes(t));
      });
    }
    return arr;
  }, [lib, search, bodyFilter]);

  useEffect(() => { setVisible(PAGE_SIZE); }, [search, bodyFilter]);

  return (
    <div>
      <GifDetailModal open={!!detail} onClose={()=>setDetail(null)} entry={detail} />
      <PH supra="Biblioteca" title={lib ? `${lib.length} ejercicios con GIF` : "Cargando..."}/>
      <Inp value={search} onChange={setSearch} placeholder="Buscar ejercicio (ej: sentadilla, curl, press)..." style={{ marginBottom:10 }}/>
      <div style={{ display:"flex", gap:6, marginBottom:16, flexWrap:"wrap" }}>
        {bodyParts.map(bp => (
          <Tag key={bp} active={bodyFilter===bp} onClick={()=>setBodyFilter(bp)}>{bp==="Todos"?"Todos":(BODY_PART_ES[bp]||bp)}</Tag>
        ))}
      </div>

      {!lib && (
        <div style={{ textAlign:"center", padding:48 }}>
          <div style={{ width:28, height:28, border:`1.5px solid ${C.dimmer}`, borderTop:`1.5px solid ${C.white}`, borderRadius:"50%", animation:"spin .8s linear infinite", margin:"0 auto 14px" }}/>
          <div style={{ fontSize:11, color:C.dimmer, letterSpacing:2, textTransform:"uppercase" }}>Cargando biblioteca...</div>
        </div>
      )}

      {lib && filtered.length===0 && (
        <Card style={{ textAlign:"center", padding:48 }}>
          <div style={{ fontSize:13, color:C.dimmer }}>Sin resultados. Probá buscar en inglés (ej: "squat", "bench press").</div>
        </Card>
      )}

      {lib && filtered.length>0 && (
        <>
          <div style={{ fontSize:11, color:C.dimmer, marginBottom:10 }}>{filtered.length} resultado{filtered.length!==1?"s":""}</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {filtered.slice(0, visible).map(e => (
              <Card key={e.id} onClick={()=>setDetail(e)} style={{ cursor:"pointer", padding:10, display:"flex", flexDirection:"column", gap:8 }}>
                <GifThumb entry={e} size="100%" />
                <div>
                  <div style={{ fontSize:12, fontWeight:600, lineHeight:1.3 }}>{e.name}</div>
                  <div style={{ fontSize:10, color:C.dim, marginTop:3 }}>{BODY_PART_ES[e.body_part]||e.body_part} · {e.equipment}</div>
                </div>
              </Card>
            ))}
          </div>
          {visible < filtered.length && (
            <Btn ghost full onClick={()=>setVisible(v=>v+PAGE_SIZE)} style={{ marginTop:14 }}>Cargar más ({filtered.length - visible} restantes)</Btn>
          )}
        </>
      )}

      <div style={{ fontSize:9, color:C.dimmer, textAlign:"center", marginTop:20 }}>{LIB_ATTRIBUTION}</div>
    </div>
  );
}
...
// ══════════════════════════════════════════════
const METRIC_FIELDS = [
  { k:"weight_kg", l:"Peso",    u:"kg", step:"0.1" },
  { k:"height_cm", l:"Altura",  u:"cm", step:"1"   },
  { k:"waist_cm",  l:"Cintura", u:"cm", step:"0.5" },
  { k:"chest_cm",  l:"Pecho",   u:"cm", step:"0.5" },
  { k:"arm_cm",    l:"Brazo",   u:"cm", step:"0.5" },
  { k:"thigh_cm",  l:"Pierna",  u:"cm", step:"0.5" },
  { k:"hip_cm",    l:"Cadera",  u:"cm", step:"0.5" },
];
function latestField(metrics, k) {
  for (const m of metrics) if (m[k]!=null && m[k]!=="") return { v:parseFloat(m[k]), date:m.date };
  return null;
}
function MiniLineChart({ points, color=C.green, height=80 }) {
  ...
  const coords = points.map((p,i) => [i*stepX, height - ((p.v-min)/range)*(height-16) - 8]);
...
function SettingsView({ user, onLogout, db, showToast }) {
  const [showForm, setShowForm] = useState(false);
  const DEF = { date:new Date().toISOString().slice(0,10), weight_kg:"", height_cm:"", waist_cm:"", chest_cm:"", arm_cm:"", thigh_cm:"", hip_cm:"", note:"" };
  const [form, setForm] = useState(DEF);
  const F = v => setForm(p=>({...p,...v}));

  const metrics = db.metrics||[];
  const lastWeight = latestField(metrics,"weight_kg");
  const lastHeight = latestField(metrics,"height_cm");
  const bmi = lastWeight&&lastHeight ? (lastWeight.v/((lastHeight.v/100)**2)) : null;

  const weightSeries = [...metrics].filter(m=>m.weight_kg!=null&&m.weight_kg!=="").sort((a,b)=>new Date(a.date)-new Date(b.date)).map(m=>({v:parseFloat(m.weight_kg), date:m.date}));
  const weightDiff = weightSeries.length>1 ? weightSeries[weightSeries.length-1].v - weightSeries[0].v : null;

  async function submit() {
    const hasAny = METRIC_FIELDS.some(f=>form[f.k]);
    if (!hasAny) { showToast("Cargá al menos un valor","err"); return; }
    const row = { date:new Date(form.date).toISOString(), note:form.note||null };
    METRIC_FIELDS.forEach(f => { row[f.k] = form[f.k]?parseFloat(form[f.k]):null; });
    await db.addMetric(row);
    setForm(DEF); setShowForm(false);
  }
  ...
  return (
    <div>
      ...
      {/* ── MÉTRICAS CORPORALES ── */}
      <div style={{ fontSize:10, letterSpacing:3, color:C.dimmer, textTransform:"uppercase", fontWeight:600, marginBottom:12, marginTop:24 }}>Métricas Corporales</div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:12 }}>
        <Card style={{ textAlign:"center", padding:"12px 6px" }}>
          <div style={{ fontFamily:FM, fontSize:20, fontWeight:700 }}>{lastWeight?lastWeight.v:"—"}</div>
          <div style={{ fontSize:9, letterSpacing:1.5, color:C.dimmer, textTransform:"uppercase", fontWeight:600, marginTop:4 }}>Peso kg</div>
        </Card>
        <Card style={{ textAlign:"center", padding:"12px 6px" }}>
          <div style={{ fontFamily:FM, fontSize:20, fontWeight:700 }}>{lastHeight?lastHeight.v:"—"}</div>
          <div style={{ fontSize:9, letterSpacing:1.5, color:C.dimmer, textTransform:"uppercase", fontWeight:600, marginTop:4 }}>Altura cm</div>
        </Card>
        <Card style={{ textAlign:"center", padding:"12px 6px" }}>
          <div style={{ fontFamily:FM, fontSize:20, fontWeight:700 }}>{bmi?bmi.toFixed(1):"—"}</div>
          <div style={{ fontSize:9, letterSpacing:1.5, color:C.dimmer, textTransform:"uppercase", fontWeight:600, marginTop:4 }}>IMC</div>
        </Card>
      </div>
      ...
      {weightSeries.length>1 && (
        <Card style={{ marginBottom:12 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            <Lbl>Evolución de peso</Lbl>
            {weightDiff!=null && <span style={{ fontFamily:FM, fontSize:13, fontWeight:700, color:weightDiff<=0?C.green:C.red }}>{weightDiff>0?"+":""}{weightDiff.toFixed(1)}kg</span>}
          </div>
          <MiniLineChart points={weightSeries}/>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:C.dimmer, marginTop:4 }}>
            <span>{fShort(weightSeries[0].date)}</span>
            <span>{fShort(weightSeries[weightSeries.length-1].date)}</span>
          </div>
        </Card>
      )}

      {!showForm && <Btn onClick={()=>setShowForm(true)} full style={{ marginBottom:16 }}>+ Registrar medidas</Btn>}

      {showForm && (
        <Card style={{ marginBottom:16, borderColor:C.b2 }}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14 }}>Nuevo registro</div>
          <Lbl>Fecha</Lbl>
          <Inp type="date" value={form.date} onChange={v=>F({date:v})} style={{ marginBottom:14 }}/>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
            {METRIC_FIELDS.map(f => (
              <div key={f.k}>
                <Lbl>{f.l} ({f.u})</Lbl>
                <Inp type="number" value={form[f.k]} onChange={v=>F({[f.k]:v})} placeholder="—"/>
              </div>
            ))}
          </div>
          <Lbl>Notas (opcional)</Lbl>
          <textarea value={form.note} onChange={e=>F({note:e.target.value})} placeholder="Cómo te sentís, contexto..." style={{ minHeight:50, marginBottom:14 }}/>
          <div style={{ display:"flex", gap:8 }}>
            <Btn onClick={submit} full>Guardar</Btn>
            <Btn ghost onClick={()=>{setShowForm(false);setForm(DEF);}} full>Cancelar</Btn>
          </div>
        </Card>
      )}
      ...
      {metrics.length>0 && (
        <div style={{ marginBottom:16 }}>
          <Lbl>Historial de registros</Lbl>
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:8 }}>
            {metrics.map(m => (
              <Card key={m.id} style={{ padding:"12px 14px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <div style={{ fontSize:12, fontWeight:600 }}>{fDate(m.date)}</div>
                  <button onClick={()=>db.deleteMetric(m.id)} style={{ background:"none", border:"none", color:C.dimmer, cursor:"pointer", fontSize:18, lineHeight:1 }}>×</button>
                </div>
                <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                  {METRIC_FIELDS.filter(f=>m[f.k]!=null&&m[f.k]!=="").map(f => (
                    <span key={f.k} style={{ fontSize:11, background:C.s2, border:`1px solid ${C.b1}`, borderRadius:6, padding:"3px 9px", fontFamily:FM }}>{f.l}: {m[f.k]}{f.u}</span>
                  ))}
                </div>
                {m.note && <div style={{ marginTop:8, fontSize:12, color:C.dimmer, fontStyle:"italic" }}>"{m.note}"</div>}
              </Card>
            ))}
          </div>
        </div>
